...
function onFormSubmit(evt) {
  evt.preventDefault();
  const imputValue = inputEl.value.trim();
  console.log("imputValue:", imputValue);
  fetch(
    // `https://www.thecocktaildb.com/api/json/v1/1/search.php?f=${imputValue}`
    `https://www.thecocktaildb.com/api/json/v1/1/search.php?s=${imputValue}`
  )
    .then((resp) => resp.json())
...
function createMarkup(array) {
  return array
    .map(({ strDrink, strCategory, strAlcoholic, strDrinkThumb }) => {
      return `
    <div class="coctailCard">
        <img src="${strDrinkThumb}"
          alt="${strDrink}"
...
function renderMarkup(array) {
    drinksListEL.insertAdjacentHTML('beforeend', createMarkup(array));
}
